...
export class TsIndexDb {
  ...
  public version: number | undefined = 1; //数据库版本
  public tableList: DbTable[] = []; //表单列表
  public db: IDBDatabase | null = null;
  private queue: (() => void)[] = []; //事务队列，实例化一次以后下次打开页面时数据库自启动

  constructor({ dbName, version }: IIndexDb) {
    this.dbName = dbName;
  ...
  query<T>({
    tableName,
    condition,
  }: Pick<DbOperate<T>, 'condition' | 'tableName'>) {
    let res: T[] = [];
    return this.commitDb<T[]>(
      tableName,
      (transaction: IDBObjectStore) => transaction.openCursor(),
...
*/
    ...
    return this.commitDb<T>(
      ...
      (transaction: IDBObjectStore) =>
        transaction
          .index(key)
          .count(
            mapCondition[countCondition.type](...countCondition.rangeValue),
          ),
      'readonly',
      (e: any, resolve: (data: T) => void) => {
  ...
  delete<T>({
    tableName,
    condition,
  }: Pick<DbOperate<T>, 'tableName' | 'condition'>) {
    let res: T[] = [];
    return this.commitDb<T>(
      tableName,
      (transaction: IDBObjectStore) => transaction.openCursor(),
  ...
  open_db() {
    return new Promise<TsIndexDb>((resolve, reject) => {
      ...
      request.onsuccess = (event: any) => {
        ...
        this.tableList = this.db?.objectStoreNames as any;
